report 50080 "INT_Fixed Asset - Analysis 2"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'rdlc/FixedAssetAnalysis.rdl';
    ...
    dataset
    {
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            Column(AVShowFAAccount; Format(AVfagroup."Acquisition Cost Account"))
            {
            }
            Column(AVShowDocument; AVShowDocument)
            {
            }
            Column(AVRetirementAmount; AVRetirementAmount)
            {
            }
            Column(AVGainLossAmt; AVGainLossAmt)
            {
            }
            ...
            begin
                ...
                CLEAR(AVFAGroup);
                AVFAGroup.SetCurrentKey(Code);
                AVFAGroup.SetRange(AVFAgroup.Code, "Fixed Asset"."FA Posting Group");
                IF AVFAGroup.FindFirst THEN;

                CLEAR(AVGainLossAmt);
                CLEAR(AVFALedger);
                AVFALedger.SetCurrentKey("FA No.");
                AVFALedger.SetRange("FA No.", "Fixed Asset"."no.");
                AVFALedger.SetRange("Depreciation Book Code", DeprBookcode);
                AVFALedger.SetRange("FA Posting Type", AVFALedger."FA Posting Type"::"Gain/Loss");
                IF AVFALedger.FindLast() THEN begin
                    repeat
                        AVGainLossAmt += AVFALedger.Amount;
                    UNTIL AVFALedger.Next = 0;
                End;

                CLEAR(AVShowDocument);
                CLEAR(AVRetirementAmount);
                IF SalesReport = TRUE THEN BEGIN
                    CLEAR(AVSaleInvL);
                    AVSaleInvL.SetCurrentKey(Type, "No.");
                    AVSaleInvL.SetRange(Type, AVSaleInvL.type::"fixed Asset");
                    AVSaleInvL.SetRange("No.", "fixed Asset"."No.");
                    IF AVSaleInvL.FindLast() THEN begin
                        AVShowDocument := AVSaleInvL."Document no.";
                        AVRetirementAmount := AVSaleInvL."Line Amount";
                    ENd;
                END;
    ...
    var
        ...
        AVFAGroup: Record "FA Posting Group";
        AVShowDocument: code[20];
        AVRetirementAmount: Decimal;
        AVGainLossAmt: Decimal;
        AVSaleInvL: Record "Sales Invoice Line";
        AVFALedger: Record "FA Ledger Entry";
